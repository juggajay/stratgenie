...
export function InvoiceReviewDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-full max-w-5xl max-h-[90vh] flex flex-col p-0 gap-0 overflow-hidden bg-slate-900/95 backdrop-blur-xl border border-white/10">
        {/* Header */}
              ...
              <span
                className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${
                  transaction.status === "draft"
                    ? "bg-amber-900/20 text-amber-400 border border-amber-500/50"
                    : transaction.status === "approved"
                    ? "bg-emerald-900/20 text-emerald-400 border border-emerald-500/50"
                    : "bg-cyan-900/20 text-cyan-400 border border-cyan-500/50"
                }`}
              >
        ...
        {isLoading ? (
          ...
          <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-[2fr_1fr] overflow-hidden">
            ...
            <div className="border-b lg:border-b-0 lg:border-r border-white/10 bg-slate-950/50 flex flex-col min-h-0">
              <div className="px-4 py-2 bg-slate-800/50 border-b border-white/10 flex-shrink-0">
              ...
              <div className="flex-1 min-h-0 overflow-auto">
                {invoice?.fileUrl ? (
                  ...
                  <div className="flex items-center justify-center h-full min-h-[300px] lg:min-h-[500px] text-slate-500">
            ...
            {/* Right: Form */}
            <div className="flex flex-col min-h-0 bg-slate-900/50">
              <div className="flex-1 overflow-y-auto p-6">
                <div className="space-y-5">
                  {/* Vendor Name */}
                  <div className="space-y-1.5">
                    <Label htmlFor="vendorName" className="text-sm font-medium text-slate-300">
                      Vendor Name
                    </Label>
                    <Input
                      id="vendorName"
                  ...
                  {/* Invoice Date */}
                  <div className="space-y-1.5">
                    <Label htmlFor="invoiceDate" className="text-sm font-medium text-slate-300">
                      Invoice Date
                    </Label>
                    <Input
                      id="invoiceDate"
                      ...
                      className="rounded-lg bg-slate-800/80 border-white/10 text-white focus:ring-cyan-500/50 focus:border-cyan-500/50"
                      disabled={!isDraft}
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        ...
                        <Input
                          ...
                          placeholder="0.00"
                          className="rounded-lg bg-slate-800/80 border-white/10 text-white placeholder:text-slate-500 pl-7 focus:ring-cyan-500/50 focus:border-cyan-500/50"
                          disabled={!isDraft}
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="gst" className="text-sm font-medium text-slate-300">
                        GST Amount
                      </Label>
                      <div className="relative">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select
                      ...
                      disabled={!isDraft}
                    >
                      <SelectTrigger className="rounded-lg bg-slate-800/80 border-white/10 text-white focus:ring-cyan-500/50 focus:border-cyan-500/50">
                        <SelectValue placeholder="Select a category" />
                      </SelectTrigger>
                      <SelectContent className="bg-slate-800 border-white/10">
                        {CATEGORIES.map((cat) => (
                          <SelectItem key={cat.value} value={cat.value} className="text-white hover:bg-white/5 focus:bg-white/10">
                            {cat.label}
                          </SelectItem>
                        ))}
                      </SelectContent>
                  ...
                  {transaction?.originalExtraction && (
                    <div className="mt-2 p-4 bg-violet-900/20 rounded-lg border border-violet-500/30">
        ...
        {/* Error Message */}
        {error && (
          <div className="mx-6 mb-4 flex items-center gap-2 text-sm text-red-400 bg-red-900/20 border border-red-500/30 px-3 py-2 rounded-lg">
        ...
        {/* Footer Actions */}
        <div className="px-6 py-4 border-t border-white/10 flex flex-col sm:flex-row justify-between items-center gap-3 bg-slate-950/50 flex-shrink-0">
          <div className="flex gap-3 w-full sm:w-auto">
            ...
            {isDraft && (
              <Button
                ...
                className="rounded-lg border-red-500/30 text-red-400 hover:bg-red-900/20"
              >
